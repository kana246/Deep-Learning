...
硬くなりたい/resistance → 耐性  
再生/回復/regeneration → 再生  
透明になりたい/invisible → 透明化  
暗闇で見える/光りたい/night vision → 暗視  

■エフェクト（その他）
水中呼吸/溺れない → 水中呼吸  
落下ダメージなし/軽やか → 低速落下  
火に強い/fire resistance/燃えない → 火炎耐性  
毒/poison → 毒  
弱体化/weakness → 弱体化  
...
鉄/iron → 鉄  
金/golden/gold → 金  
ダイヤ/diamond/dia → ダイヤモンド  
ネザライト/netherite → ネザライト  
────────────────────────

【アイテム構造の変換】
・「素材 + アイテム」は必ず「[素材]の[アイテム]」へ統一  
...
1. 必ず「[対象]に」から文を始める  
2. 「あげる/やる/渡す/くれ」 → 「与える」  
3. 「〜したい/〜になりたい」 → 効果付与（対象＝自分）  
4. 複数要求は「、」で区切る  
5. 否定構文（「いらない」「渡すな」など）は無視して出力しない  
6. 例外的入力でも必ず構造化された1文にまとめる  
────────────────────────

【禁止事項】
・説明文を返さない  
・文末に余分な文章をつけない  
・Minecraft に存在しないアイテム名を創作しない  
────────────────────────
...
# ========== コマンド生成画面 ==========
    ...
    if generate_btn and user_input:
        ...
        if st.session_state.generation_mode == 'both':
            ...
            if f'generation_data_{generation_id}' not in st.session_state:
                st.session_state[f'generation_data_{generation_id}'] = {
            ...
            # 送信済みの場合はメッセージを表示
            if st.session_state.get(f'submitted_{generation_id}', False):
                st.info("✅ このデータは既に送信済みです")
