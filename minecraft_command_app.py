...
──────────────────────── 【出力形式】 必ず次のいずれかの形式で出力してください。複数の場合は「、」で区切り1行で出力。

[対象]に[アイテム名]を[数量]個与える（give, items）

[対象]に[効果名]の効果を付ける（effect）

[場所]に[モブ名]を[数量]体召喚する（summon）
...
[対象]を[座標または目的地]へテレポートさせる（tp）

ゲームモードを[モード名]に変更する（gamemode）

[場所形式]にスポーン地点を設定する（setworldspawn, spawnpoint）
...
「朝にして」→「時間を朝にする」と出力（「晴れ」と混同しないこと）。

「ゾンビを消して」→「全対象（またはゾンビ）を消去する」。
